...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  return {
    title: `${product.name} в ${city.name} - ${product.seoTitle}`,
    description: product.seoDescription,
    keywords: `${product.name.toLowerCase()} ${city.name}, натяжные потолки ${city.name}, ${product.textures.join(', ')}, ${product.types.join(', ')}`,
    openGraph: {
      title: `${product.name} в ${city.name} - ПОТОЛКИ`,
      description: product.seoDescription,
      type: 'website',
      locale: 'ru_RU',
      images: [
    ...
    alternates: {
      canonical: `https://potolki.ru/${city.slug}/catalog/${product.id}`
    }
  };
}
